...
## Context
We need to detect exactly what changed in a note file during an editing session, so we can show the user a precise diff and decide whether to commit.

We considered several approaches:
- **A**: Hash-based comparison with snapshot files (store pre-edit hash and full content)
- **B**: Delta-based diff tracking (track line-by-line changes in real-time during edit)
